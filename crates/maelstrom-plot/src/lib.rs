...
/// User-requested modifications to the plot bounds. We collect them in the plot build function to
/// later apply them at the right time, as other modifications need to happen first.
...
/// closure provided to [`Plot::show`]. See [`Plot`] for an example of how to use it.
pub struct PlotUi {
...
impl PlotUi {
    ...
    /// The plot bounds as they were in the last frame. If called on the first frame and the bounds
    /// were not further specified in the plot builder, this will return bounds centered on the
    /// origin. The bounds do not change until the plot is drawn.
    ...
    /// The pointer position in plot coordinates. Independent of whether the pointer is in the plot
    /// area.
    pub fn pointer_coordinate(&self) -> Option<PlotPoint> {
        // We need to subtract the drag delta to keep in sync with the frame-delayed screen
        // transform:
...
///
/// This function should return 3 positive step sizes, designating where the lines in the grid are
/// drawn.  Lines are thicker for larger step sizes. Ordering of returned value is irrelevant.
///
/// Why only 3 step sizes? Three is the number of different line thicknesses that egui typically
/// uses in the grid.  Ideally, those 3 are not hardcoded values, but depend on the visible range
/// (accessible through `GridInput`).
